...
- **最適化アルゴリズム**: OR-Tools CP-SAT Solver（制約充足問題）
- **制約条件の厳格な適用**: 連勤制限、月間勤務日数、資格者配置などを完全遵守
- **実行時間制限なし**: GASの6分制限から解放
- **診断機能**: INFEASIBLE時に原因分析と対策を出力（v2.1）

#### Python スクリプト一覧

| ファイル | 説明 |
|----------|------|
| `shift_optimizer.py` | 基本版（全グループ一括処理） |
| `shift_optimizer_diagnostic.py` | 診断機能付き版（推奨） |

#### 診断機能付き版の特徴（v2.1）

1. **事前診断（Pre-flight Check）**
   - グループ人数の過不足チェック
   - 夜勤可能者の人数チェック
   - 喀痰吸引資格者の有無確認
   - 勤務枠の需給バランス検証
...
3. **診断モード設定**
   ...
   | 設定 | デフォルト | 説明 |
   |------|-----------|------|
   | `ENABLE_PARTIAL_OUTPUT` | True | 成功グループのみ出力（部分出力モード） |
   | `RELAXED_MODE` | False | 制約を緩和して解を探索（緊急用） |

4. **部分出力モード**
   - 一部グループが失敗しても、成功グループのみ `シフト結果_YYYYMM_partial.csv` として出力
   - 失敗グループは手動でシフト作成
...
5. **制約緩和モード**（緊急用）
   ...
   | 早出人数 | 2名以上 | 1名以上 |
   | 日勤人数 | 1名以上 | 0名OK |
   | 所定勤務日数 | 21日ぴったり | 19〜23日（±2日） |
   | 喀痰吸引資格者 | 毎日必須 | 不要 |

6. **出力ファイル**
   - `シフト結果_YYYYMM.csv`: 全グループ成功時
   - `シフト結果_YYYYMM_partial.csv`: 部分成功時
   - `診断レポート_YYYYMM.json`: 診断結果（エラー・警告・改善提案）
...
| ファイル | 行数 | 役割 |
|----------|------|------|
| shift_optimizer.py | ~656 | 基本版シフト最適化（全グループ一括処理） |
| shift_optimizer_diagnostic.py | ~1061 | 診断機能付き版（グループ別処理、エラー分析、部分出力対応） |
...
### Python最適化でINFEASIBLEエラー

**原因:** 制約条件を満たすシフトが数学的に存在しない

**対処法:**
1. `shift_optimizer_diagnostic.py`（診断機能付き版）を使用
2. 診断レポートで失敗原因を確認
3. よくある原因と対策：

   | 原因 | 対策 |
   |------|------|
   | グループ人数不足（5名未満） | 職員を増員するか、他グループから異動 |
   | 夜勤可能者不足 | 勤務配慮を外すか、夜勤可能な職員を追加 |
   | 休み希望の集中 | 優先順位の低い希望を調整依頼 |
   | 喀痰吸引資格者不在 | 資格者を追加 |

4. 緊急対応: `RELAXED_MODE = True` で制約を緩和（手動調整必須）

### Python「ModuleNotFoundError: No module named 'ortools'」
...
| バージョン | 日付 | 変更内容 |
|-----------|------|----------|
| v2.1 | 2025-01-05 | 診断機能付きPythonスクリプト追加（shift_optimizer_diagnostic.py） |
